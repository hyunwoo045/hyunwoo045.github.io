---
title: "[ElasticSearch] npm @elastic/elasticsearch - CRUD (1)"
excerpt: "메모용"
...
`@elastic/elasticsearch` 의 7.13.0 버전을 설치함. 2022년 1월 17일 기준으로 `the client noticed that the server is not a supported distribution of elasticsearch` 이런 에러를 만났는데, 7.16.0 버전을 쓰니 나왔다. 바로 다운그레이드.
...
진짜 짜증나는데 아직도 일단은 얘도 비슷하다. `update` 가 있고 `updateByQuery` 가 있다. 이하 생략
...
body 를 케이크처럼 쉽게 먹기 위해서는 doc 이라는 field 를 넣어주자.
